...
* Используется все тот же мод на автоперевод [XUnity.AutoTranslator](https://github.com/bbepis/XUnity.AutoTranslator). (если есть альтернативы лучше - предлагайте, ток без гемороя)
...
1. Скачайте готовый архив и шрифт из релизов и распакуйте в папку с игрой. Или скачайте обновленные файлы и конфиги перевода (и анценза) из репо, если можете вручную их раскидать куда надо.
   Примеч. Структура файлов в коде репо упрощена(без кучи лишних папок) и не совсем равна структуре установки (в отличии от релиза).
...
### Примеч. Доп инфа.
Русик и пр. предоставляется как есть. Перераспространение не запрещено, кроме продажи (я же не продаю чужой перевод 😑). Модификация распространяется бесплатно, так что продажа запрещена (если вы купили, то вас нае*али, а я пока все еще сижу без доната, да и никто и не предлалагает..).
...
* Если вы хотите заменить/обновить базовый файл перевода, но чтоб модификации продолжали работать, попробуйте переименовать его(большой файл перевода) обратно в `_Auto.txt`(если имя уже не такое).
